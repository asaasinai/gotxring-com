...
type GroupWithItems = Prisma.ConfigOptionGroupGetPayload<{ include: { items: { orderBy: { sortOrder: 'asc' } } } }>;

function ItemForm({ groupId, item }: { groupId: string; item?: GroupWithItems['items'][0] }) {
  return (
    <form action={upsertConfigItemAction} className="grid gap-2 rounded-lg border border-zinc-800 bg-black/30 p-3">
      ...
      <button className="btn-primary w-fit text-sm">{item ? 'Save Item' : 'Add Item'}</button>
    </form>
  );
}
...
function GroupSection({ group }: { group: GroupWithItems }) {
  return (
    <div className="rounded-xl border border-zinc-700 bg-zinc-900/40 p-5 grid gap-5">
      {/* Group header + edit form */}
      <form action={upsertConfigGroupAction} className="grid gap-3">
        ...
        <div className="grid gap-3 sm:grid-cols-[1fr_1fr_auto_auto]">
          ...
          <div>
            <label className="label">Sort Order</label>
            <input className="input" name="sortOrder" type="number" defaultValue={group.sortOrder} />
          </div>
          ...
          <div className="flex items-end">
            <button className="btn-primary text-sm">Save Group</button>
          </div>
        </div>
        <div>
    ...
    <div className="grid gap-8">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-400">
          Configure the option groups shown on the Full Rifle System order form. Each group is one selection step.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-dashed border-zinc-700 p-5">
        ...
        <form action={upsertConfigGroupAction} className="grid gap-3">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div>
              <label className="label">Sort Order</label>
              <input className="input" name="sortOrder" type="number" defaultValue={groups.length + 1} />
            </div>
          </div>
          <div>
            <label className="label">Description</label>
          ...
          <button className="btn-primary w-fit">Create Group</button>
        </form>
      </div>
    </div>
  );
